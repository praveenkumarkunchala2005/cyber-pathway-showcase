  ...
  return (
    <div className={`cyber-card p-5 transition-all duration-300 hover:scale-[1.02] ${isActive ? 'border-cyber-glow/30 animate-glow' : ''}`}>
      <div className="flex items-start">
        <div className="bg-cyber-light/30 p-2 rounded-full mr-4 backdrop-blur-sm">
          <GraduationCap className="h-6 w-6 text-cyber-glow" />
        </div>
        <div className="flex-1">
          <h3 className={`text-xl font-space font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-800'} mb-1`}>
            {institution}
          </h3>
          <p className={`text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'} mb-2`}>{degree}</p>
